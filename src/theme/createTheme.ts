...
export const createTheme = (theme: Partial<TypeTheme>): TypeTheme => {
  ...
  const successColors = generateColorVariants(
    theme.palette?.success || DEFAULT_VALUES.PALETTE.SUCCESS,
    theme.palette?.tonalOffset
  );
  const dangerColors = generateColorVariants(
    theme.palette?.danger || DEFAULT_VALUES.PALETTE.DANGER,
    theme.palette?.tonalOffset
  );
  const softColors = generateColorVariants(
    theme.palette?.soft || DEFAULT_VALUES.PALETTE.SOFT,
    theme.palette?.tonalOffset
  );
  ...
  const getPaletteValue = (key: keyof TypePalette, fallback: string | number) =>
    (theme.palette?.[key] || defaultPalette?.[key] || fallback) as string;
  const getBorderRadiusValue = (key: keyof TypeBorderRadius, ratio: number) =>
    theme.borderRadius?.[key] ?? baseBorderRadius * ratio;
  const getPaddingValue = (key: keyof TypeSpacing, ratio: number) =>
    theme.padding?.[key] ?? baseGap * ratio;
  const getGapValue = (key: keyof TypeSpacing, ratio: number) =>
    theme.gap?.[key] ?? basePadding * ratio;
  ...
  const finalTheme: TypeTheme = {
    palette: {
      ...
      inputPlaceholder: getPaletteValue(
        "inputPlaceholder",
        DEFAULT_VALUES.PALETTE.INPUT_PLACEHOLDER
      ),

      tonalOffset: getPaletteValue(
        "tonalOffset",
